...
const StudentsScreen = () => {
  ...
  const fetchAnalytics = async () => {
    setLoading(true);
    try {
      const [analyticsResponse, overallResponse] = await Promise.all([
        apiService.getStudentAnalytics(),
        apiService.getOverallAnalytics()
      ]);

      if (analyticsResponse?.data && overallResponse?.data) {
        setStudentAnalytics(analyticsResponse.data);
        setOverallAnalytics(overallResponse.data);
      } else {
  ...
  const renderSelectedStudentDetails = () => {
    if (!selectedStudent || !studentAnalytics) return null;

    const student = studentAnalytics.find((s: any) => s.name === selectedStudent);
    if (!student) return null;

    return (
      <View style={styles.analyticsCard}>
        <Text style={styles.chartTitle}>Student Details</Text>
        <Text style={styles.analyticsValue}>{student.attendance_percentage}%</Text>
        <Text style={styles.analyticsLabel}>Overall Attendance</Text>
        <View style={styles.detailsRow}>
          <Text>Total Classes: {student.total_classes}</Text>
          <Text>Classes Attended: {student.attended_classes}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <ScrollView style={styles.screen}>
      ...
      {studentAnalytics && overallAnalytics ? (
        <View style={styles.chartsContainer}>
          <View style={styles.chartCard}>
            <Text style={styles.chartTitle}>Attendance Distribution</Text>
            <BarChart
              data={{
                labels: studentAnalytics.map((student: any) => student.name),
                datasets: [{
                  data: studentAnalytics.map((student: any) => student.attendance_percentage)
                }]
              }}
              width={350}
              height={220}
          ...
          <View style={styles.chartCard}>
            ...
            <LineChart
              data={{
                labels: overallAnalytics.attendance_trend.map((day: any) => day.date),
                datasets: [{
                  data: overallAnalytics.attendance_trend.map((day: any) => day.attendance_rate)
                }]
              }}
              width={350}
              height={220}
          ...
          <View style={styles.chartCard}>
            ...
            <PieChart
              data={[
                {
                  name: 'Green',
                  population: overallAnalytics.zone_distribution.green,
                ...
                {
                  name: 'Yellow',
                  population: overallAnalytics.zone_distribution.yellow,
                ...
                {
                  name: 'Red',
                  population: overallAnalytics.zone_distribution.red,
          ...
          <View style={styles.pickerContainer}>
            ...
            {selectedStudent && renderSelectedStudentDetails()}
          </View>
        ) : (
          <View style={styles.loadingContainer}>
      ...
      const TimetableScreen = () => {
        ...
        const fetchTimetable = async () => {
          try {
            ...
            console.log('Fetched timetable:', response.data.timetable);
